...
describe('PropertyDetailPageComponent comparables', () => {
  it("charge les comparables au changement d'onglet valorisation", async () => {
    ...
    expect(
      component.sliderPercent(
        component.comparableTargetSurfaceM2(),
        surfaceSlider.min,
        surfaceSlider.max,
      ),
    ).not.toBeNull();
    ...
    const coproCategory = component
      .propertyCategories()
      .find((category) => category.id === 'copropriete');
    ...
    const amenitiesCategory = component
      .propertyCategories()
      .find((category) => category.id === 'amenities');
  ...
  it("permet de mettre a jour le prix de vente et de relancer l'analyse IA", async () => {
    ...
    expect(
      component.valuationKeyCriteria().some((criterion) => criterion.field.key === 'dpeClass'),
    ).toBe(true);
    expect(
      component.valuationKeyCriteria().some((criterion) => criterion.field.key === 'pool'),
    ).toBe(true);
    expect(
      component.valuationKeyCriteria().some((criterion) => criterion.field.key === 'standing'),
    ).toBe(true);
  ...
  it('affiche le champ fosse aux normes uniquement pour fosse septique et nettoie la valeur sinon', async () => {
    ...
    expect(
      (component.property()?.details as Record<string, unknown>)['septicTankCompliant'],
    ).toBeNull();
  });
  ...
  it("persiste l'année de dernière rénovation et les détails complémentaires agent", async () => {
    ...
    expect((component.property()?.details as Record<string, unknown>)['lastRenovationYear']).toBe(
      2020,
    );
    expect(
      (component.property()?.details as Record<string, unknown>)['agentAdditionalDetails'],
    ).toBe('Façade rénovée, cuisine refaite, isolation des combles.');
  });
  ...
  it("reactualise le nuage de points avec 2 % d'inflation par an", async () => {
    vi.spyOn(Date, 'now').mockReturnValue(new Date('2026-03-20T00:00:00.000Z').getTime());

    const propertyResponse: PropertyResponse = {
      id: 'property_inflation',
      title: 'Appartement Nice',
      city: 'Nice',
      postalCode: '06000',
      address: '8 avenue Jean Medecin',
      price: 125000,
      details: {
        general: {
          propertyType: 'APPARTEMENT',
    ...
    const comparablesResponse: PropertyComparablesResponse = {
      ...
      windowYears: 10,
      search: {
        center: { latitude: 43.7, longitude: 7.26 },
        finalRadiusM: 1200,
        radiiTried: [1200],
        targetCount: 100,
        targetReached: true,
      ...
      points: [
        {
          saleDate: '2021-03-20T00:00:00.000Z',
          surfaceM2: 50,
          landSurfaceM2: null,
          salePrice: 100000,
          pricePerM2: 2000,
        ...
        {
          saleDate: '2026-03-20T00:00:00.000Z',
          surfaceM2: 60,
          landSurfaceM2: null,
          salePrice: 120000,
          pricePerM2: 2000,
      ...
      providers: [
        {
          ...
          useValue: {
            snapshot: {
              paramMap: convertToParamMap({ id: 'property_inflation' }),
            },
          },
        },
        { provide: PropertyService, useValue: propertyServiceMock },
    ...
    expect(component.comparablesChartInflationEnabled()).toBe(false);
    expect(component.comparablesChartPriceLabel()).toBe('Prix');
    expect(component.comparablesChartYAxisLabel()).toBe('Prix (€)');
    expect(component.chartComparablePoints()[0]?.chartSalePrice).toBe(100000);
    expect(component.comparablesChartPriceSummary().minPrice).toBe(100000);

    component.setComparablesChartInflationEnabled(true);

    const expectedAdjustedPrice = Number(
      (
        100000 *
        Math.pow(
          1.02,
          (Date.now() - new Date('2021-03-20T00:00:00.000Z').getTime()) /
            (365.25 * 24 * 60 * 60 * 1000),
        )
      ).toFixed(2),
    );

    expect(component.comparablesChartInflationEnabled()).toBe(true);
    expect(component.comparablesChartPriceLabel()).toBe('Prix réactualisé');
    expect(component.comparablesChartYAxisLabel()).toBe('Prix réactualisé (€)');
    expect(component.chartComparablePoints()[0]?.chartSalePrice).toBeCloseTo(
      expectedAdjustedPrice,
      2,
    );
    expect(component.comparablesChartPriceSummary().minPrice).toBeCloseTo(expectedAdjustedPrice, 2);
    expect(component.filteredComparablePoints()[0]?.salePrice).toBe(100000);
  });
  ...
  it('calcule la progression des types documentaires et persiste les documents attendus masques', async () => {
    ...
    expect(firstComponent.expectedDocumentsForActiveTab().map((item) => item.provided)).toEqual([
      true,
      true,
    ]);
    ...
    expect(firstComponent.expectedDocumentsForActiveTab().some((item) => item.index === 0)).toBe(
      false,
    );
  ...
  it('évalue les computed principaux et couvre les flux upload/vocal', async () => {
    ...
    const subjectPoint = (
      internals['resolveSubjectPointForChart'] as (
    ...
    expect(
      component.rentalProfitability().irrPct === null ||
        (component.rentalProfitability().irrPct ?? 0) > 0,
    ).toBe(true);
    expect(component.inseeModule().city === null || component.inseeModule().city === 'Grasse').toBe(
      true,
    );
    ...
    expect(
      component.filteredComparablePoints().some((point) => point.salePrice === 4_700_000),
    ).toBe(true);
    expect(component.filteredComparablePoints().some((point) => point.salePrice === 40_000)).toBe(
      false,
    );
    expect(component.chartComparablePoints().some((point) => point.salePrice === 4_700_000)).toBe(
      false,
    );
    ...
    component.onUploadBackdropClick({
      target: 'same',
      currentTarget: 'same',
    ...
    component.onVocalBackdropClick({
      target: 'same',
      currentTarget: 'same',
    } as unknown as MouseEvent);
    ...
    expect((internals['parseOptionalNumber'] as (value: string) => number | null)('12,5')).toBe(
      12.5,
    );
    expect((internals['parseOptionalNumber'] as (value: string) => number | null)('x')).toBeNull();
    expect(
      (internals['parsePositiveNumber'] as (value: unknown) => number | null)('1 200'),
    ).toBeNull();
    expect(
      (internals['parseComparableSaleTimestamp'] as (value?: string) => number | null)(undefined),
    ).toBeNull();
    expect(
      (internals['formatComparableSaleDate'] as (value?: string) => string | null)(
        '2024-01-01T00:00:00.000Z',
      ),
    ).not.toBeNull();
    expect(
      (internals['normalizeEmptyAsNull'] as (value: string) => string | null)('   '),
    ).toBeNull();
    expect(
      (internals['toIsoFromDateTimeInput'] as (value: string) => string | null)('bad'),
    ).toBeNull();
    expect((internals['parseBooleanDetail'] as (value: unknown) => boolean | null)('true')).toBe(
      true,
    );
    expect((internals['parseBooleanDetail'] as (value: unknown) => boolean | null)(' FALSE ')).toBe(
      false,
    );
    ...
    expect(
      (
        (internals['resolveSelectedProspectClient'] as () => { id: string } | null)() ?? {
          id: null,
        }
      ).id,
    ).toBe('client_1');
    ...
    expect(
      ((internals['resolveSelectedVisitClient'] as () => { id: string } | null)() ?? { id: null })
        .id,
    ).toBe('client_1');

    expect(
      (internals['toCanonicalHiddenExpectedDocumentKey'] as (key: string) => string | null)(''),
    ).toBeNull();
    expect(
      (internals['toCanonicalHiddenExpectedDocumentKey'] as (key: string) => string | null)(
        'legacy-key',
    ...
    expect(
      (internals['toControlValue'] as (value: unknown, field: unknown) => string)(true, boolField),
    ).toBe('true');
    expect(
    ...
    expect(
      (internals['toControlValue'] as (value: unknown, field: unknown) => string)('foo', boolField),
    ).toBe('');
    expect(
      (internals['toControlValue'] as (value: unknown, field: unknown) => string)(
    ...
    (
      internals['assignPropertyPatchValue'] as (
        payload: unknown,
        key: string,
        value: string,
      ) => void
    )(patchPayload, 'title', 'Titre MAJ');
    (
      internals['assignPropertyPatchValue'] as (
        payload: unknown,
        key: string,
    ...
    )(patchPayload, 'address', '7 avenue des Fleurs');
    (
      internals['assignPropertyPatchValue'] as (
        payload: unknown,
        key: string,
